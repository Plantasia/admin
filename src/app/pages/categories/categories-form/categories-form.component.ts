...
export class CategoriesFormComponent implements OnInit, AfterViewInit {
  ...
  private buildCategoryForm() {
    this.categoryForm = new FormGroup({
      ...
      authorEmail: new FormControl("", [Validators.minLength(10)]),

      created_at: new FormControl("", [Validators.minLength(10)]),

      updated_at: new FormControl("", [Validators.minLength(10)]),

      deleted_at: new FormControl("", [Validators.minLength(10)]),
  ...
  private handleFileInput(event) {
    if (event.target.files.length > 0) {
      ...
      this.service
        .imageUpload(file, this.category.id)
        .pipe(
          catchError((error) => {
            console.log(error);
            return undefined;
  ...
  private createCategory() {
    const newCategory: CategoryModel = Object.assign(
      new CategoryModel(),
      this.categoryForm.value
    );

    this.service.create(newCategory).subscribe((c) => {
      console.log(c);
